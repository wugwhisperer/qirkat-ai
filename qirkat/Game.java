...
class Game {
    ...
    void process() {
        ...
        while (true) {
            while (_state == SETUP || _state == null) {
                doCommand();
            }
    ...
    void doLoad(String[] operands) {
        try {
            ...
            ReaderSource src = new ReaderSource(reader, false);
    ...
    void doDump(String[] unused) {
        String output = _board.toString(false);
        System.out.println("===");
    ...
    void reportWinner() {
        String msg;
        msg = board().whoseMove().opposite().toString() + " wins.";
